...
Delimiter //
Create procedure sp_FacturaAtencion( in codFacturaB int)
	Begin
		select 
		...
		from Facturas F
			join Empleados E on F.codigoEmpleado = E.codigoEmpleado
                ...
                where F.codigoFactura = codFacturaB;
    End//
Delimiter ;
call sp_FacturaAtencion();
-- drop procedure sp_FacturaAtencion;
...
Delimiter //
Create procedure sp_ReporteUltimaConsulta()
	Begin
		select M.nombreMascota as 'Nombre Mascota', C.nombreCliente as 'Nombre Cliente', Co.motivo as 'Motivo de consulta'
			from Mascotas M 
				join Consultas Co on Co.codigoMascota = M.codigoMascota
					join Clientes C on C.codigoCliente = M.codigoCliente
						where Co.codigoConsulta = (select max(codigoConsulta) from Consultas);
    End//
Delimiter ;
call sp_ReporteUltimaConsulta();

/*2.	Mostrar el nombre del veterinario, el nombre del cliente y el nombre de la mascota atendida en cada cita completada.*/
Delimiter //
Create procedure sp_CitaCompletada()
	Begin
		select V.nombreVeterinario as 'Nombre Veterinario', C.nombreCliente as 'Nombre Cliente', M.nombreMascota as 'Nombre Mascota', Ci.estado as 'Estado'
			from Citas Ci
				join Veterinarios V on Ci.codigoVeterinario = V.codigoVeterinario
					join Mascotas M on Ci.codigoMascota = M.codigoMascota
						join Clientes C on M.codigoCliente = C.codigoCliente
							where Ci.estado like 'Completa';
    End//
Delimiter ;
call sp_CitaCompletada();

/*3.	Listar las recetas indicando el medicamento, la consulta a la que pertenece y el veterinario que la prescribió.*/
Delimiter //
Create procedure sp_ListadoRecetaVeterinario()
	Begin
    select R.codigoMedicamento as 'Codigo Medicamento' , M.nombre as 'Nombre Medicamento', C.codigoConsulta as 'Codigo Consulta', V.nombreVeterinario as 'Nombre Veterinarion'
		from Recetas R
			join Consultas C on R.codigoConsulta = C.codigoConsulta
				join Veterinarios V on C.codigoVeterinario = V.codigoVeterinario
					join Medicamentos M on R.codigoMedicamento = M.codigoMedicamento;
    End//

Delimiter ;

call sp_ListadoRecetaVeterinario();

/*4.	Mostrar las facturas emitidas, con nombre del cliente, nombre del empleado que las procesó y total facturado.*/
Delimiter //
Create procedure sp_FacturasEmitidas()
	Begin
		select F.codigoFactura as 'Codigo Factura', C.nombreCliente as 'Nombre Cliente', E.nombreEmpleado as 'Nombre Empleado', F.total as 'Total Facturado'
			from Facturas F 
				join Clientes C on F.codigoCliente = C.codigoCliente
					join Empleados E on F.codigoEmpleado = E.codigoEmpleado;
	End//
Delimiter ;
call sp_FacturasEmitidas();

/*5.	Consultar los tratamientos realizados con su diagnóstico relacionado y el veterinario que lo atendió.*/
Delimiter //
Create procedure sp_ConsultaDeTratamientos()
	Begin
		select T.codigoTratamiento as 'Codigo Tratamiento', C.diagnostico 'Diagnostico', V.nombreVeterinario as 'Nombre Veterinario'
			from Tratamientos T 
				join Consultas C on T.codigoConsulta = C.codigoConsulta
					join Veterinarios V on C.codigoVeterinario = V.codigoVeterinario;
	End//
Delimiter ;
call sp_ConsultaDeTratamientos();

/*6.	Listar las vacunaciones indicando el nombre de la mascota, el nombre del veterinario y el nombre de la vacuna aplicada.*/
Delimiter //
Create procedure sp_Vacunaciones()
	Begin
		select V.codigoVacunacion as 'Codigo Vacunacion', M.nombreMascota as 'Nombre Mascota', Ve.nombreVeterinario as 'Nombre Veterinario', Va.nombreVacuna as'Vacuna aplicada'
			from Vacunaciones V
				join Mascotas M on V.codigoMascota = M.codigoMascota
					join Veterinarios Ve on V.codigoVeterinario = Ve.codigoVeterinario
						join Vacunas Va on V.codigoVacuna = Va.codigoVacuna;
	End//
Delimiter ;
call sp_Vacunaciones();

/*7.	Reportar las compras de medicamentos, indicando nombre del proveedor, nombre del medicamento y fecha de compra.*/

Delimiter //
	Create procedure sp_ComprasMedicamentos()
		Begin
			select P.nombreProveedor as 'Nombre Proveedor', Me.nombre as 'Nombre Medicamento', C.fechaCompra as 'Fecha de Compra'
				from Proveedores P
					join Medicamentos Me on P.codigoProveedor = Me.codigoProveedor
						join Compras C on P.codigoProveedor = C.codigoProveedor;
		End//
Delimiter ;
call sp_ComprasMedicamentos();

/*8.	Mostrar la informacion de las mascotas con recetas generadas.*/

Delimiter //
	Create procedure sp_MascotaRecetaGenerada()
		Begin
			select M.codigoMascota as 'Codigo Mascota', M.nombreMascota as 'Nombre Proveedor', especie as 'Especie', raza as 'Raza', sexo as 'Sexo', fechaNacimiento as 'Fecha Nacimiento',
				color as 'Color', pesoActualKg as 'Peso', codigoCliente as 'CodigoCliente'
				from Recetas R
					join Consultas C on R.codigoConsulta = C.codigoConsulta
						join Mascotas M  on C.codigoMascota = M.codigoMascota
							where  codigoReceta >=1;
		End//
Delimiter ;

call sp_MascotaRecetaGenerada();

/*9.	Obtener un reporte de las citas pendientes con datos del cliente, la mascota y el veterinario asignado.*/

Delimiter //
Create procedure sp_CitasPendientes()
	Begin
		select C.codigoCliente as 'Codigo Cliente',concat(C.nombreCliente, ' ', C.apellidoCliente) as'Nombre Cliente', C.telefonoCliente as 'Telefono Cliente',
		C.direccionCliente as 'Direccion Cliente', C.emailCliente as 'Email Cliente', C.fechaRegistro as 'Fecha Registro' ,M.codigoMascota as 'Codigo Mascota',
		M.nombreMascota as 'Nombre Proveedor', M.especie as 'Especie', M.raza as 'Raza', M.sexo as 'Sexo', M.fechaNacimiento as 'Fecha Nacimiento',M.color as 'Color', M.pesoActualKg as 'Peso',
		V.codigoVeterinario as 'Codigo Veterinario',concat(V.nombreVeterinario, ' ', V.apellidoVeterinario) as 'Nombre Veterinario' , V.especialidad as 'Especialidad', V.telefonoVeterinario as 'TelefonoVeterinario'
		, V.correoVeterinario as 'CodigoVeterinario', V.fechaIngreso as 'Fecha Ingreso', V.estado as 'Estado'
			from Citas Ci
				join Veterinarios V on Ci.codigoVeterinario= V.codigoVeterinario
					join Mascotas M  on Ci.codigoMascota = M.codigoMascota
						join Clientes C on  M.codigoCliente = C.codigoCliente
...
call sp_CitasPendientes();

/*10.	Mostrar el historial de consultas de cada mascota, incluyendo el veterinario y diagnóstico correspondiente.*/
Delimiter //
Create procedure sp_HistrorialConsulta()
Begin
	select C.codigoConsulta as 'Codigo Consulta', M.codigoMascota as 'Codigo Mascota', M.nombreMascota as 'Nombre Mascota', V.nombreVeterinario as 'Nombre Veterinario', C.diagnostico as 'Diagnostico'
		from Consultas C
			join Mascotas M  on C.codigoMascota = M.codigoMascota
				join Veterinarios V on C.codigoVeterinario = V.codigoVeterinario;
End//
...
call sp_HistrorialConsulta();

/*11.	Generar un reporte de vacunas aplicadas, indicando fecha, mascota y proveedor del medicamento (vacuna).*/

Delimiter //
Create procedure sp_VacunasAplicadas()
	Begin
		select V.fechaAplicacion as 'Fecha Aplicacion', M.nombreMascota as 'Nombre Mascota', P.nombreProveedor as 'Proveedor'
			from Mascotas M
				join Vacunaciones V  on M.codigoMascota = V.codigoMascota
					join Consultas C on M.codigoMascota = C.codigoMascota
						join Recetas R on C.codigoConsulta = R.codigoConsulta
							join Medicamentos Me on R.codigoMedicamento = Me.codigoMedicamento
								join Proveedores P on Me.codigoProveedor = P.codigoProveedor;
	End//
Delimiter ;


call sp_VacunasAplicadas();

/*12.	Mostrar todas las consultas realizadas, vinculando la mascota, el cliente y el veterinario.*/
Delimiter //
	Create procedure sp_ConsultasRealizadas()
		Begin
			Select Con.codigoConsulta as 'Codigo Consulta', M.nombreMascota as Mascota, C.nombreCliente as Cliente, V.nombreVeterinario as Veterinario
				from Consultas Con
					join Veterinarios V on Con.codigoVeterinario = V.codigoVeterinario
						join Mascotas M on Con.codigoMascota = M.codigoMascota
							join Clientes C on M.codigoCliente = C.codigoCliente;
		End//
Delimiter ;

call sp_ConsultasRealizadas();
...
Create procedure sp_RecetasMedicas()
	Begin
		select Me.descripcion as 'Descripcion Medicamento', T.descripcion as 'Tratamiento Descripcion', V.nombreVeterinario as 'Nombre Veterinario'
			from Consultas C
				join Tratamientos T  on C.codigoConsulta = T.codigoTratamiento
					join Veterinarios V on C.codigoVeterinario = V.codigoVeterinario
						join Recetas R on C.codigoConsulta = R.codigoConsulta
							join Medicamentos Me on R.codigoMedicamento = Me.codigoMedicamento;
	End//
...
call sp_RecetasMedicas();

/*14.	Listar todas las facturas, el cliente asociado, el empleado que la emitió y la mascota consultada relacionada.*/
Delimiter //
Create procedure sp_FacturaClientesAsociados()
	Begin
		Select F.codigoFactura as 'Codigo Factura', C.nombreCliente as 'Cliente', E.nombreEmpleado as 'Empleado', M.nombreMascota as 'Mascota'
				from Facturas F
					join Clientes C on F.codigoCliente = C.codigoCliente
						join Empleados E on F.codigoEmpleado = E.codigoEmpleado
							join Mascotas M on C.codigoCliente = M.codigoCliente;
	End//
Delimiter ;
call sp_FacturaClientesAsociados;
/*15.	Reportar todas las consultas, mostrando cliente, mascota y los medicamentos recetados en cada una.*/
Delimiter //
Create procedure sp_ConsultasRecetas()
	Begin
		select C.nombreCliente as 'Nombre Cliente',M.nombreMascota as 'Nombre Mascota', Me.nombre as 'Medicamento'
			from Mascotas M 
				join Clientes C on C.codigoCliente = M.codigoCliente
					join Consultas Con on M.codigoMascota = Con.codigoMascota
						join Recetas R on Con.codigoConsulta = R.codigoConsulta
							join Medicamentos Me on Me.codigoMedicamento = R.codigoMedicamento;
	End//
Delimiter ;
call sp_ConsultasRecetas();